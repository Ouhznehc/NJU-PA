...
//    Log("%s", envp[envc]);
  ...
  Log("argsz: %d, envsz: %d", argsz, envsz);
  ...
  size_t totsz = argsz + envsz + sizeof(uintptr_t) * (envc + argc + 3);

  Log("n_area: %p, n_envp: %p, n_argv: %p, n_argc: %p", n_area, n_envp, n_argv, n_argc);
  ...
  *n_argc = argc;
  ...
  pcb->cp->GPRx = (uintptr_t) (pcb->as.area.end - totsz);
  Log("Successfully Loaded user process %s", filename);
  Log("Stack starts at pa %p and va %p", n_argc, (pcb->as.area.end - totsz));
  Log("Context at %p", pcb->cp);
}
