...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(dest) = SEXT((Mr(src1 + imm, 2)),16));
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(dest) = Mr(src1 + imm, 2));
  INSTPAT("000000? ????? ????? 001 ????? 00100 11", slli   , I, if(((imm >> 5) & 1) == 0) R(dest) = src1 << (imm & 0x1f));
  INSTPAT("000000? ????? ????? 101 ????? 00100 11", srli   , I, if(((imm >> 5) & 1) == 0) R(dest) = src1 >> (imm & 0x1f));
  INSTPAT("??????? ????? ????? 110 ????? 00100 11", ori    , I, R(dest) = src1 | imm);
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti   , I, R(dest) = ((int)src1 < (int)imm));
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(dest) = SEXT((Mr(src1 + imm, 1)),8));
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , I, s->dnpc = isa_raise_intr(1, s->pc));
 // INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, R(dest) = CSRS(imm, 0, 0); CSRS(imm, CSRS(imm, 0, 0) | src1, 1););
 // INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, R(dest) = CSRS(imm, 0, 0); CSRS(imm, src1, 1););
  ...
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(dest) = ((uint64_t)src1 * (uint64_t)src2) >> 32);
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, R(dest) = src1 / src2);
 // INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , R, s->dnpc = CSRS(0x341, 0, 0));
