...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  //TODO INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , I, R(dest) = (int)src1 >> (imm & 0x1f));
  INSTPAT("??????? ????? ????? 101 ????? 00100 11", andi   , I, R(dest) = src1 & imm);
  //TODO INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(dest) = Mr(src1 + imm, 4));
