...
void SDL_BlitSurface(SDL_Surface *src, SDL_Rect *srcrect, SDL_Surface *dst, SDL_Rect *dstrect) {
  ...
  if(dst->format->BitsPerPixel == 32){
    ...
    for(int i = 0; i < h; i++){
      for(int j = 0; j < w; j++){
        dst_pixels[(i + dy) * dst->w + dx + j] = src_pixels[(i + y) * src->w + x + j];
      }
    }
  }
  else{
...
uint32_t Shift_bits(SDL_Color color){
  return ((uint32_t)color.r << 16) | ((uint32_t)color.g << 8) | ((uint32_t)color.b << 0) | ((uint32_t)color.a << 24);
}
...
void SDL_UpdateRect(SDL_Surface *s, int x, int y, int w, int h) {
  if(s->format->BitsPerPixel == 32){
    if(x ==0 && y == 0 && w == 0 && h == 0){
      ...
      for(int i = y; i <= y + h; i++)
        memcpy(&pixels[(i - y) * w], &s->pixels[i * s->w + x], sizeof(uint32_t) * w);
      NDL_DrawRect(pixels, x, y, w, h);
      free(pixels);
    }
  }
    ...
    for(int i = y; i < y + h; i++)
      for(int j = 0; j < w; j++)
        pixels[(i - y) * w + j] = Shift_bits(s->format->palette->colors[s->pixels[i * s->w + x + j]]);
    NDL_DrawRect(pixels, x, y, w, h);
    free(pixels);
  }
}
